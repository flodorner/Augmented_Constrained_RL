...
def main(alpha=0.02,add_penalty=1,keep_add_penalty=True,mult_penalty=None,cost_penalty=None,filename=""):
    if mult_penalty == -1:
        mult_penalty = None
    if cost_penalty == -1:
        cost_penalty = None
    env = gym.make('Safexp-PointGoal1-v0')
    env = constraint_wrapper(env,add_penalty=add_penalty,keep_add_penalty=keep_add_penalty,mult_penalty=mult_penalty,cost_penalty=cost_penalty)
...
if __name__ == "__main__":
    ...
    filename = "sac_constraint_a_"+str(args.alpha).replace(".","_")+"_pen_"+str(args.add_penalty).replace(".","_")\
               +args.keep_add_penalty*"_s"+"_mult_"+str(args.mult_penalty).replace(".","_")+"_cost_"+str(args.cost_penalty).replace(".","_")
    sys.stdout = open("results/"+filename+".txt", 'w')
    main(args.alpha,args.add_penalty,args.keep_add_penalty,args.mult_penalty,args.cost_penalty,filename)
